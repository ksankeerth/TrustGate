...
# PNet/RNet/ONet weights are small (~2MB total) and ship as bundled package
# data upstream, but are fetched here instead so nothing binary needs to be
# committed to this repo -- same pattern as every other model weight in this
# project. Verified (sha256, see app/layers/_vendor/facenet_pytorch/README.md)
# to be byte-identical to the files bundled in facenet-pytorch's own PyPI wheel.
_MTCNN_WEIGHT_URLS = {
    'pnet': (
        'https://raw.githubusercontent.com/timesler/facenet-pytorch/master/data/pnet.pt',
        'a2a71925e0b9996a42f63e47efc1ca19043e69558b5c523b978d611dfae49c8f',
    ),
    'rnet': (
        'https://raw.githubusercontent.com/timesler/facenet-pytorch/master/data/rnet.pt',
        'bbb937de72efc9ef83b186c49f5f558467a1d7e3453a8ece0d71a886633f6a86',
    ),
    'onet': (
        'https://raw.githubusercontent.com/timesler/facenet-pytorch/master/data/onet.pt',
        '165bfbe42940416ccfb977545cf0e976d5bf321f67083ae2aaaa5c764280118d',
    ),
}
...
def _load_mtcnn_weights(name):
    url, digest = _MTCNN_WEIGHT_URLS[name]
    model_dir = os.path.join(_get_torch_home(), 'checkpoints')
    os.makedirs(model_dir, exist_ok=True)
    cached_file = os.path.join(model_dir, f'{name}.pt')
    if not os.path.exists(cached_file):
        download_url_to_file(url, cached_file, hash_prefix=digest)
...
class PNet(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            state_dict = _load_mtcnn_weights('pnet')
...
class RNet(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            state_dict = _load_mtcnn_weights('rnet')
            self.load_state_dict(state_dict)
...
class ONet(nn.Module):
    ...
    def __init__(self, pretrained=True):
        ...
        if pretrained:
            state_dict = _load_mtcnn_weights('onet')
            self.load_state_dict(state_dict)
